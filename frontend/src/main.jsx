...
// Auto-theme: sample the background artwork and set CSS variables so panels harmonize with the image.
function setThemeFromBackgroundImage() {
  try {
    const cssBg = getComputedStyle(document.documentElement).getPropertyValue('--bg-image') || '';
    const m = cssBg.match(/url\((['"]?)(.*?)\1\)/);
    const url = m ? m[2] : cssBg.trim().replace(/['"]/g, '');
    if (!url) return;
    ...
    img.onload = () => {
      ...
      function darken(rgb, amt){ return rgb.map(v=>Math.max(0, Math.round(v * (1 - amt)))); }
      function luminance([r,g,b]){
        const srgb = [r,g,b].map(v=>v/255).map(c=> c <= 0.03928 ? c/12.92 : Math.pow((c+0.055)/1.055, 2.4));
        return 0.2126*srgb[0] + 0.7152*srgb[1] + 0.0722*srgb[2];
      }

      const top = avgRegion(0, Math.floor(H/3));
      const mid = avgRegion(Math.floor(H/3), Math.floor(2*H/3));
      const bot = avgRegion(Math.floor(2*H/3), H);

      // Assign variables: mid -> primary panel, bot -> muted, top -> accent
      document.documentElement.style.setProperty('--panel-1', toHex(mid));
      document.documentElement.style.setProperty('--panel-2', toHex(bot));
      document.documentElement.style.setProperty('--panel-3', toHex(top));
      document.documentElement.style.setProperty('--panel-4', toHex(darken(mid, 0.55)));

      // Decide readable text color for panels based on luminance
      const midLum = luminance(mid);
      const textOnMid = midLum > 0.5 ? '#111111' : '#ffffff';
      document.documentElement.style.setProperty('--panel-text-dark', '#111111');
      document.documentElement.style.setProperty('--panel-text-light', '#ffffff');
      // For panel-1 (primary), prefer good contrast
      const panel1Text = midLum > 0.5 ? '#111111' : '#ffffff';
      document.documentElement.style.setProperty('--panel-text', panel1Text);
    };
    img.onerror = () => {/* ignore failures; fall back to defaults */};
  } catch (e) {
    // graceful fallback
