...
function Report({ report }) {
  ...
  return <div className="report">
    ...
    <section className="report-section"><h3>Overview</h3><div className="metric-list">
      {[['Dimensions', width && height ? `${width} × ${height}` : '—'], ['Data type', report.image_stats?.dtype || '—'], ['Brightness', report.mean_brightness], ['Contrast', report.contrast_score], ['Sharpness', report.sharpness_score], ['Colorfulness', report.colorfulness_score], ['Entropy', report.entropy_score], ['Exposure', `${format(report.underexposed_pct)}% under · ${format(report.overexposed_pct)}% over`]].map(([name, value]) => <div key={name}><span>{name}</span><strong>{typeof value === 'string' ? value : format(value)}</strong></div>)}
    </div></section>
